...
void KX_GameObject::UpdateActivity(float distance)
{
	...
	// Manage animation culling.
	if (m_activityCullingInfo.m_flags & ActivityCullingInfo::ACTIVITY_ANIMATION && m_actionManager) {
		if (distance > m_activityCullingInfo.m_animationRadius) {
...
void KX_GameObject::SetActivityCulling(ActivityCullingInfo::Flag flag, bool enable)
{
	if (enable) {
		...
		if (flag & ActivityCullingInfo::ACTIVITY_ANIMATION && m_actionManager) {
			m_actionManager->Resume();
		}
	}
}
...
PyAttributeDef KX_GameObject::Attributes[] = {
	...
	KX_PYATTRIBUTE_RW_FUNCTION("animationCullingRadius", KX_GameObject, pyattr_get_animationCullingRadius, pyattr_set_animationCullingRadius),
	...
	KX_PYATTRIBUTE_RW_FUNCTION("animationCulling", KX_GameObject, pyattr_get_animationCulling, pyattr_set_animationCulling),
...
PyObject *KX_GameObject::pyattr_get_animationCulling(PyObjectPlus *self_v, const KX_PYATTRIBUTE_DEF *attrdef)
{
	KX_GameObject *self = static_cast<KX_GameObject *>(self_v);
	return PyBool_FromLong(self->GetActivityCullingInfo().m_flags & ActivityCullingInfo::ACTIVITY_ANIMATION);
}

int KX_GameObject::pyattr_set_animationCulling(PyObjectPlus *self_v, const KX_PYATTRIBUTE_DEF *attrdef, PyObject *value)
{
	KX_GameObject *self = static_cast<KX_GameObject *>(self_v);
	int param = PyObject_IsTrue(value);
	if (param == -1) {
	...
	self->SetActivityCulling(ActivityCullingInfo::ACTIVITY_ANIMATION, param);
	return PY_SET_ATTR_SUCCESS;
}
...
PyObject *KX_GameObject::pyattr_get_animationCullingRadius(PyObjectPlus *self_v, const KX_PYATTRIBUTE_DEF *attrdef)
{
	KX_GameObject *self = static_cast<KX_GameObject *>(self_v);
	return PyFloat_FromDouble(std::sqrt(self->GetActivityCullingInfo().m_animationRadius));
}

int KX_GameObject::pyattr_set_animationCullingRadius(PyObjectPlus *self_v, const KX_PYATTRIBUTE_DEF *attrdef, PyObject *value)
{
	KX_GameObject *self = static_cast<KX_GameObject *>(self_v);
	const float val = PyFloat_AsDouble(value);
	if (val < 0.0f) { // Also accounts for non float.
	...
	self->GetActivityCullingInfo().m_animationRadius = val * val;
