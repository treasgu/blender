	...
	m_showShadowFrustum(KX_DebugOption::DISABLE)
{
	...
	m_renderQueries[QUERY_SAMPLES].reset(new RAS_Query(RAS_Query::SAMPLES));
	m_renderQueries[QUERY_PRIMITIVES].reset(new RAS_Query(RAS_Query::PRIMITIVES));
	m_renderQueries[QUERY_TIME].reset(new RAS_Query(RAS_Query::TIME));
...
void KX_KetsjiEngine::RenderDebugProperties()
{
	...
	if (m_flags & SHOW_RENDER_QUERIES) {
		...
		for (unsigned short i = 0; i < QUERY_MAX; ++i) {
			...
			if (i == QUERY_TIME) {
				debugtxt = (boost::format("%5.2fms") % (((float)m_renderQueries[i]->Result()) / 1e6)).str();
			}
			else {
